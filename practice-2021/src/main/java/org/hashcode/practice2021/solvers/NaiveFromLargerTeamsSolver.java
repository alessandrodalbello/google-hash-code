...
public class NaiveFromLargerTeamsSolver implements Solver<PizzeriaInput, PizzeriaOutput> {
    ...
    private List<Delivery> chooseDeliveries(List<Pizza> pizzas, int a, int b, int c) {
        ...
        while ((linkedPizzas.size() >= 4 && (a >= 1 || b >= 1 || c >= 1))
                || (linkedPizzas.size() >= 3 && (b >= 1 || c >= 1))
                || (linkedPizzas.size() >= 2 && c >= 1)) {
            int teamSize;
            if (a >= 1) {
                teamSize = 4;
                a -= 1;
            ...
            assert linkedPizzas.size() >= teamSize;
